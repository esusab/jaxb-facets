...
public class XmlSchemaEnhancer {
	...
	private static boolean instanceOf(Annotation annoInst,
		...
		if(Proxy.isProxyClass(annoInst.getClass())) {
			AnnotationInvocationHandler h = AnnotationUtils.PROXY_HANDLERS.get(annoInst);
			if(h != null) {
	...
	private static <T, C> Object getAnnotationOfProperty(
		...
		if (info.parent().getType() instanceof ClassType || info.parent().getType().getClass().getName().equals("com.sun.tools.javac.code.Type$ClassType")) {
		...
		for (Field f : parent.getDeclaredFields()) {
			if (f.getName().equals(fieldName)) {
				...
				if (getAnnotation(f, XmlAttribute.class) != null) {
					XmlAttribute a = getAnnotation(f,
							XmlAttribute.class);
